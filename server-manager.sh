...
# Copper Kingdom Plugin Server Manager
...
online-mode=false
white-list=false
motd=Copper Kingdom Plugin Test Server
view-distance=10
...
EOF
    ...
    print_status "Note: Use 'make give-copper' for testing materials"
}
...
show_status() {
    ...
    if [[ -f "$SERVER_DIR/plugins/${PLUGIN_NAME}-0.1.0.jar" ]]; then
        print_success "Copper Kingdom plugin is installed"
    else
        print_warning "Copper Kingdom plugin is not installed"
    fi
}
...
case "${1:-help}" in
    ...
    players)
        ...
        ;;
    help|*)
        echo "Copper Kingdom Plugin Server Manager"
        echo ""
